...
class NeuralNetwork():
    ...
    def fprop(self, batch_input, train=True):
        ...
        for l in range(1, self.num_layers + 1): 
            ...
            if l != self.num_layers :
                ...
                self.net['D%s' % l] =  np.random.rand(self.net['Z%s' % l].shape[0], self.net['Z%s' % l].shape[1])   
                # Step 2: convert entries of D1 to 0 or 1 (using keep_prob as the threshold)
                self.net['D%s' % l] = self.net['D%s' % l] <  self.keep_prob           
                self.net['Z%s' % l] = np.multiply(self.net['Z%s' % l] , self.net['D%s' % l])   # Step 3: shut down some neurons of A1
                self.net['Z%s' % l] = self.net['Z%s' % l] / self.keep_prob   # Step 4: scale the value of neurons that haven't been shut down   
    ...
    def update_parameters(self, epoch, t):
        ...
        if self.optimizer == 'sgd':
            ...
            for l in range(1,self.num_layers + 1):
                ...
                self.parameters['b%s' % l] = self.parameters['b%s' %l] - np.divide((self.learning_rate * self.velocity['db%s' % l]) , (np.sqrt(self.scale['db%s' % l]) + eps))
                t+=1
    ...
    def bprop(self, batch_target):
        ...
        for l in range(self.num_layers-1 , 0,-1):
            self.net['dA%s' %l] = np.matmul(self.parameters['W%s' %(l+1)].T ,self.net['dZ%s'%(l+1)])
            self.net['dA%s' %l] = np.multiply(self.net['dA%s' %l] , self.net['D%s' % l])
            self.net['dA%s' %l] = self.net['dA%s' %l] / self.keep_prob
            ...
            self.net['dZ%s'%l] = self.net['dA%s' %l] * derivate
